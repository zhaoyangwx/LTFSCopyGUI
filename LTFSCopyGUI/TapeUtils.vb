...
#End Region
    ...
    ' Reuses the SCSI_PASS_THROUGH_DIRECT control packet for the ordered
    ' ExternalReader write stream. This type is deliberately single-threaded:
    ' a Rust slot must remain leased until its matching write completes.
    Friend NotInheritable Class FastSequentialWriteSession
        Implements IDisposable

        Private Const MaximumWriteLength As UInteger = &HFFFFFFUI
        Private Const WriteTimeoutSeconds As UInteger = 900UI

        Private ReadOnly _handle As IntPtr
        Private ReadOnly _directIo As Boolean
        Private ReadOnly _sense(IOCtl.SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER.SendBufferLength - 1) As Byte
        Private ReadOnly _zeroSense(IOCtl.SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER.SendBufferLength - 1) As Byte
        Private ReadOnly _ownerThreadId As Integer
        Private _packet As IntPtr
        Private _packetSize As UInteger
        Private _scsiStatusOffset As Integer
        Private _dataTransferLengthOffset As Integer
        Private _dataBufferOffset As Integer
        Private _cdbOffset As Integer
        Private _senseOffset As Integer
        Private _disposed As Boolean

        Public Sub New(handle As IntPtr)
            If handle = IntPtr.Zero OrElse handle = New IntPtr(-1) Then Throw New ArgumentException("Invalid tape handle", NameOf(handle))
            _handle = handle
            _ownerThreadId = Threading.Thread.CurrentThread.ManagedThreadId
            _directIo = DriverTypeSetting <> DriverType.TapeStream
            If Not _directIo Then Return

            Dim template = IOCtl.BuildSCSIPassThroughStructure(
                {&HA, 0, 0, 0, 0, 0},
                IntPtr.Zero,
                0UI,
                0,
                WriteTimeoutSeconds)
            _packetSize = CUInt(Marshal.SizeOf(template))
            Dim sptOffset = CInt(Marshal.OffsetOf(GetType(IOCtl.SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER), NameOf(IOCtl.SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER.Spt)))
            _scsiStatusOffset = sptOffset + CInt(Marshal.OffsetOf(GetType(IOCtl.SCSI_PASS_THROUGH_DIRECT), NameOf(IOCtl.SCSI_PASS_THROUGH_DIRECT.ScsiStatus)))
            _dataTransferLengthOffset = sptOffset + CInt(Marshal.OffsetOf(GetType(IOCtl.SCSI_PASS_THROUGH_DIRECT), NameOf(IOCtl.SCSI_PASS_THROUGH_DIRECT.DataTransferLength)))
            _dataBufferOffset = sptOffset + CInt(Marshal.OffsetOf(GetType(IOCtl.SCSI_PASS_THROUGH_DIRECT), NameOf(IOCtl.SCSI_PASS_THROUGH_DIRECT.DataBuffer)))
            _cdbOffset = sptOffset + CInt(Marshal.OffsetOf(GetType(IOCtl.SCSI_PASS_THROUGH_DIRECT), NameOf(IOCtl.SCSI_PASS_THROUGH_DIRECT.Cdb)))
            _senseOffset = CInt(Marshal.OffsetOf(GetType(IOCtl.SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER), NameOf(IOCtl.SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER.Sense)))

            _packet = Marshal.AllocHGlobal(CInt(_packetSize))
            Try
                Marshal.StructureToPtr(template, _packet, False)
            Catch
                Marshal.FreeHGlobal(_packet)
                _packet = IntPtr.Zero
                Throw
            End Try
        End Sub

        Public Function Write(data As IntPtr, length As UInteger) As Byte()
            If _disposed Then Throw New ObjectDisposedException(NameOf(FastSequentialWriteSession))
            If data = IntPtr.Zero Then Throw New ArgumentNullException(NameOf(data))
            If length = 0 OrElse length > MaximumWriteLength Then Throw New ArgumentOutOfRangeException(NameOf(length))
            Diagnostics.Debug.Assert(Threading.Thread.CurrentThread.ManagedThreadId = _ownerThreadId,
                                     "FastSequentialWriteSession must stay on its creating thread")

            If Not _directIo Then Return TapeUtils.Write(_handle, data, length, True)

            Array.Clear(_sense, 0, _sense.Length)
            Marshal.Copy(_zeroSense, 0, IntPtr.Add(_packet, _senseOffset), _zeroSense.Length)
            Marshal.WriteByte(_packet, _scsiStatusOffset, 0)
            Marshal.WriteInt32(_packet, _dataTransferLengthOffset, CInt(length))
            Marshal.WriteIntPtr(_packet, _dataBufferOffset, data)
            Marshal.WriteByte(_packet, _cdbOffset + 2, CByte((length >> 16) And &HFFUI))
            Marshal.WriteByte(_packet, _cdbOffset + 3, CByte((length >> 8) And &HFFUI))
            Marshal.WriteByte(_packet, _cdbOffset + 4, CByte(length And &HFFUI))
            ...
            RaiseFastIOCtlStart()
            Try
                result = DeviceIoControl(_handle,
                                         ...
                                         bytesReturned,
                                         IntPtr.Zero)
                If Not result Then errorCode = GetLastError()
            Finally
                RaiseFastIOCtlFinished()
            End Try
            If Not result Then
                Dim win32ex As New Win32Exception(errorCode)
                Throw New Exception($"SCSI Failure. {vbCrLf}ErrCode: 0x{errorCode:X8}h{vbCrLf}{win32ex.Message}")
            End If
            Marshal.Copy(IntPtr.Add(_packet, _senseOffset), _sense, 0, _sense.Length)
            Return _sense
        End Function
        ...
        Public Sub Dispose() Implements IDisposable.Dispose
            ...
            _disposed = True
            If _packet <> IntPtr.Zero Then
                Marshal.DestroyStructure(_packet, GetType(IOCtl.SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER))
                Marshal.FreeHGlobal(_packet)
                _packet = IntPtr.Zero
            End If
    ...
    Private Shared Sub RaiseFastIOCtlStart()
        RaiseEvent IOCtlStart()
    End Sub

    Private Shared Sub RaiseFastIOCtlFinished()
        RaiseEvent IOCtlFinished()
    End Sub
                ...
                Try
                    ...
                    If ExtraPartitionCount > 0 Then
                        ...
                        If DriverTypeSetting <> DriverType.TapeStream Then
                            ...
                            If TapeUtils.SendSCSICommand(handle:=handle, cdbData:=New Byte() {&H15, &H10, 0, 0, &H10, 0}, Data:=New Byte() {0, 0, &H10, 0, &H11, &HA, CByte(MaxExtraPartitionAllowed), 1, ModeData(4), ModeData(5), 9, ModeData(7), CByte((P0Size >> 8) And &HFF), CByte(P0Size And &HFF), CByte((P1Size >> 8) And &HFF), CByte(P1Size And &HFF)}, DataIn:=0, senseReport:=senseReportFunc) Then
